...
# --- Block Loading Editor Window Class (Phase 1) ---
class BlockEditorWindow(tk.Toplevel):
    def __init__(self, parent, callback, initial_data=None):
        ...
        # Each dict represents a block and will hold its levels and properties
        if initial_data:
            self.blocks = initial_data
        else:
            self.blocks = [{'name': 'Block 1', 'levels': []}]
    ...
    def _create_widgets(self):
        ...
        # --- Left Pane: Block List and Controls ---
        ...
        self.block_listbox = tk.Listbox(list_frame)
        ...
        # --- Right Pane: Editor for Selected Block ---
        self.right_pane = ttk.LabelFrame(paned_window, text="Edit Selected Block", padding=10)
        paned_window.add(self.right_pane, weight=3)
        
        # Placeholder for now
        ttk.Label(self.right_pane, text="Stress level editor will appear here.").pack(pady=20)
        
    def _populate_block_listbox(self):
        """Clears and repopulates the list of blocks."""
        current_selection = self.block_listbox.curselection()
        
        self.block_listbox.delete(0, tk.END)
        for i, block in enumerate(self.blocks):
            self.block_listbox.insert(tk.END, f"Block {i+1}")

        # Re-select the previous item if possible
        if current_selection:
            new_index = min(current_selection[0], len(self.blocks) - 1)
            if new_index >= 0:
                self.block_listbox.selection_set(new_index)
                self.block_listbox.activate(new_index)
        elif self.blocks:
             self.block_listbox.selection_set(0)
             self.block_listbox.activate(0)
    ...
    def _on_block_select(self, event=None):
        ...
        if not selection_indices:
            self.right_pane.config(text="No Block Selected")
            return
        
        selected_index = selection_indices[0]
        self.right_pane.config(text=f"Edit Block {selected_index + 1}")

    def _add_block(self):
        new_block_num = len(self.blocks) + 1
        self.blocks.append({'name': f'Block {new_block_num}', 'levels': []})
        self._populate_block_listbox()
    ...
    def _move_block_up(self):
        ...
        self.block_listbox.activate(index - 1)

    def _move_block_down(self):
        selection = self.block_listbox.curselection()
        if not selection or selection[0] >= len(self.blocks) - 1: return
        index = selection[0]
        self.blocks[index], self.blocks[index+1] = self.blocks[index+1], self.blocks[index]
        self._populate_block_listbox()
        self.block_listbox.selection_set(index + 1)
    ...
    def _save_and_close(self):
        # The logic to pass data back to the main GUI will be added here
        self.destroy()
...
class FastranGui(tk.Tk):
    ...
    def _create_tab1(self):
        ...
        # --- Create all conditional buttons. Their visibility is handled by _update_all_states ---
    ...
    def _create_tab3(self):
        ...
        # --- Primary Loading Frame and its conditional children ---
        self.ca_loading_frame = ttk.Frame(self.lf17)
        self._create_entry_row(self.ca_loading_frame, "Max Stress (SMAXP):", "SMAXP", 0, 0, tooltip_text="Maximum applied stress for the primary loading block.")
        self._create_entry_row(self.ca_loading_frame, "Min Stress (SMINP):", "SMINP", 0, 1, tooltip_text="Minimum applied stress for the primary loading block.")
        self._create_entry_row(self.ca_loading_frame, "Cycles (NCYCP):", "NCYCP", 1, 0, tooltip_text="Number of cycles for the primary loading block.")
        self.ca_loading_frame.pack(fill='x')

        self.standard_loading_frame = ttk.Frame(self.lf17)
        self._create_entry_row(self.standard_loading_frame, "MAXSEQ:", "MAXSEQ", 0, 0, tooltip_text="Total number of blocks or flights in the sequence to be repeated.")
        self._create_entry_row(self.standard_loading_frame, "MAXBLK:", "MAXBLK", 0, 1, tooltip_text="Number of different kinds of blocks or flights.")
        self._create_entry_row(self.standard_loading_frame, "LPRINT:", "LPRINT", 1, 0, tooltip_text="Print option for spectrum information.")
        self._create_entry_row(self.standard_loading_frame, "MAXLPR:", "MAXLPR", 1, 1, tooltip_text="Number of flights/blocks to be printed out.")
        self.speak_lbl, self.speak_entry_main = self._create_entry_row(self.standard_loading_frame, "SPEAK:", "SPEAK", 2, 0, tooltip_text="Highest stress in the spectrum, used for scaling.")
        self.smean_lbl, self.smean_entry = self._create_entry_row(self.standard_loading_frame, "SMEAN:", "SMEAN", 2, 1, tooltip_text="Mean stress for TWIST, Mini-TWIST, or Gaussian spectra.")
        self.nrep_lbl, self.nrep_ent = self._create_entry_row(self.standard_loading_frame, "NREP:", "NREP", 3, 0, tooltip_text="Number of spectrum repetitions (for NFOPT=8 only).")
        self.marker_lbl, self.marker_ent = self._create_entry_row(self.standard_loading_frame, "MARKER:", "MARKER", 3, 1, tooltip_text="Starting point marker in spectrum (for NFOPT=8 only).")
        ...
        # --- Pack all top-level frames in the desired order ---
    ...
    def _update_all_states(self, *args):
        # --- Update NALP-dependent widgets (Section 8) ---
        if self.vars['NALP_DESC'].get() == self.nalp_rev_map.get('1'):
            self.lf8.grid()
        else:
            self.lf8.grid_remove()
        
        # --- Update IRATE-dependent widgets (NGC, CRKNGC) ---
        ...
        # --- Update NFOPT-dependent widgets ---
        ...
        # --- Updated logic for dynamic buttons ---
        if is_file_based:
            self.browse_spec_button.grid()
        ...
        if is_file_based:
            ...
            self.browse_spec_button.grid_remove()
            self.edit_spec_button.grid_remove()
            self.edit_spec_button.config(state='disabled')
            self.spectrum_full_path = None
        ...
        # --- Update NTYP/LTYP-dependent widgets ---
        ...
        # --- Update KTH-dependent widgets (Section 18) ---
    ...
    def _open_block_editor(self):
        ...
        # We will pass data to/from this editor later
        BlockEditorWindow(self, None, initial_data=None)
