...
def parse_fastran_file(filepath):
    try:
        ...
        if 0 <= nfopt_val <= 10 and line_idx < len(lines) and "HALT" not in lines[line_idx].upper():
            ...
            # --- Logic for Primary Loading ---
            # NFOPT 0 and 1 both use the block-loading format according to the user guide.
            if nfopt_val in [0, 1]:
                # Read SCALE value
...
def generate_fastran_file(values, table_data, block_data, save_path, maps):
    try:
        ...
        if 0 <= nfopt_val <= 10:
            ...
            # NFOPT 0 and 1 both use the block-loading format.
            if nfopt_val in [0, 1]:
...
class FastranGui(tk.Tk):
    ...
    def _setup_help_content(self):
        """Parses the main help string into a dictionary for use in tooltips."""
        self.help_content = {}
        # This is a copy of the help string from the _show_help method
        HELP_CONTENT = """
        OUTPUT_FILE: The name of the FASTRAN input file to be created.
        SPECTRA: The name of the spectrum file (e.g., spectrum.txt). Required for NFOPT=4 through 10.
        MAT: A descriptive name for the material being analyzed.
        SYIELD: The 0.2% offset yield strength of the material.
        SULT: The ultimate tensile strength of the material.
        E: The modulus of elasticity.
        ETA: Poisson's ratio.
        ALP: The constraint factor, alpha. Use 1.0 for plane stress and ~2.5 for plane strain.
        BETAT: Compressive yielding factor for the intact material.
        BETAW: Compressive yielding factor for the material in the crack wake.
        NALP: Option for plane-strain to plane-stress transition. 0 for constant constraint, 1 for variable.
        NEP: Option for plasticity model. 1 for plasticity-corrected analysis (default), 0 for elastic analysis.
        IRATE: Number of crack-growth rate relations to be used.
        NGC: Enables (1) or disables (0) the small-to-large crack transition.
        CRKNGC: The crack length where small-to-large crack transition occurs.
        C1: Crack-growth coefficient C1.
        C2: Crack-growth power C2.
        C3: Threshold constant C3.
        C4: Threshold constant C4.
        C5: Cyclic fracture toughness, C5.
        C6: Power on fracture term, C6.
        C7: Power on threshold term, C7.
        KF: Elastic-plastic fracture toughness, KF.
        M: Fracture toughness parameter (0 <= m <= 1).
        NEQN: Selects the crack-growth rate equation (0 for FASTRAN, 1 for NASGRO).
        NTAB: If > 1, indicates the number of points for tabular crack growth input.
        NDKTH: Defines how the crack growth table is used.
        RATE1: Crack-growth rate at the start of the flat-to-slant transition.
        ALP1: Constraint factor (alpha) at RATE1.
        BETAT1: Compressive yielding factor (beta_t) at RATE1.
        BETAW1: Compressive wake yielding factor (beta_w) at RATE1.
        RATE2: Crack-growth rate at the end of the flat-to-slant transition.
        ALP2: Constraint factor (alpha) at RATE2.
        BETAT2: Compressive yielding factor (beta_t) at RATE2.
        BETAW2: Compressive wake yielding factor (beta_w) at RATE2.
        NIPT: Print interval. Results are printed every NIPT calculated points.
        NPRT: Print option. Results are printed for every NPRT-th point saved.
        LSTEP: Cycle counting option for variable-amplitude loading (typically 1).
        NDKE: Option for printing effective stress-intensity factors.
        DCPR: Crack-length increment for printing results. Overrides NIPT/NPRT if > 0.
        NTYP: Code that defines the geometry of the cracked component.
        LTYP: Code for loading type (0=tension, 1=bending, 2=combined).
        LFAST: Selects the crack-closure model.
        NS: Number of elements used to model the notch-root radius (typically 1).
        INVERT: For spectrum loads, inverts the sign of all Smin values if set to 1.
        KCONST: Loading control option (0=constant stress, 1=constant SIF range).
        NTCMAX: Maximum number of cycles for any level in a flight.
        KTAB: Number of stress-intensity factor (SIF) data pairs for user-input table.
        GAMMA: Ratio of outer fiber bending stress to remote tensile stress.
        RADIUS: Radius of a pressurized cylinder.
        RIVETS: Rivet pitch or linear spacing for a lap-splice joint.
        RLF1: Rivet load factor (0 to 1).
        RLF2: By-pass load factor (1 to 0).
        DELTA: Change in rivet radius (interference).
        NODKL: Option for rivet-load decay due to crack growth.
        W: One-half width of the specimen.
        T: Specimen thickness.
        CI: Initial crack length.
        AI: Initial crack depth for surface/corner cracks.
        CF: Final crack length at which the analysis terminates.
        RAD: Radius of a circular hole or semi-circular edge notch.
        CN: Length of the starter notch.
        AN: Depth of the starter notch.
        HN: Half-height of the starter notch.
        RADF: Radius of a fastener (set to 0 for an open hole).
        SMAX: Maximum stress for pre-cracking.
        SMIN: Minimum stress for pre-cracking.
        NRC: Special option code, usually 0.
        DVALUE: Value used with the NRC option.
        NCYCLE1: Start cycle for stress history output.
        NCYCLE2: End cycle for stress history output.
        MAXSEQ: Maximum number of sequences or blocks to be run.
        MAXBLK: Maximum number of total blocks (or flights) to be run.
        LPRINT: Print option for variable-amplitude load history.
        MAXLPR: Maximum number of load levels to be printed.
        SPEAK: Peak stress in a spectrum, used as a multiplier.
        SMEAN: Mean stress for TWIST and Mini-TWIST spectra.
        NREP: Number of repetitions of a spectrum (NFOPT=8).
        MARKER: Marker code for the last point in a repeated block (NFOPT=8).
        SMAXP: Maximum applied stress for constant-amplitude loading.
        SMINP: Minimum applied stress for constant-amplitude loading.
        NCYCP: Number of cycles for constant-amplitude loading.
        KTH: Selects the load-reduction threshold test type.
        SMAXTH: Initial maximum stress for the start of the threshold test.
        RTH: Stress ratio (Smin/Smax) for the threshold test.
        CONST: Constant K-gradient (C) for KTH=1, or dK/da for KTH=2.
        PRT: Load-reduction percentage per step for KTH=3.
        """
        current_key = None
        current_text = []
        for line in HELP_CONTENT.strip().split('\n'):
            line = line.strip()
            if not line: continue
            if ':' in line:
                if current_key:
                    self.help_content[current_key] = ' '.join(current_text)
    ...
    def _create_widgets(self):
        ...
        # This line is modified to call the new post-processor launcher
    ...
    def _create_entry_with_button(self, parent, label_text, var_key, row, command, btn_text):
        """Helper to create a label, entry, and button in a grid row."""
        lbl = ttk.Label(parent, text=label_text)
        lbl.grid(row=row, column=0, sticky='w', padx=5, pady=5)
        
        entry = ttk.Entry(parent, textvariable=self.vars[var_key], width=40)
        entry.grid(row=row, column=1, sticky='ew', padx=5)
        
        btn = ttk.Button(parent, text=btn_text, command=command)
        btn.grid(row=row, column=2, padx=5)
        
        ToolTip(lbl, self.help_content.get(var_key, ''))
        
        return lbl, entry, btn
    
    def _browse_save(self):
        """Opens a 'save as' dialog and sets the main input/output path."""
        filepath = filedialog.asksaveasfilename(
            title="Set FASTRAN Input File Path",
            initialfile=self.vars['OUTPUT_FILE'].get() or "fastran_input.txt",
            defaultextension=".txt",
            filetypes=(("Text Files", "*.txt"), ("All Files", "*.*"))
        )
        if filepath:
    ...
    def _browse_spec_file(self):
        """Opens a file dialog to select a spectrum file."""
        filepath = filedialog.askopenfilename(
            title="Select Spectrum File",
            filetypes=(
                ("Supported Spectra", "*.txt *.spx *.sub"),
                ("All Files", "*.*")
            )
        )
        if filepath:
    ...
    def _create_tab1(self):
        """Creates widgets for Tab 1: Main inputs."""
        canvas = tk.Canvas(self.tab1, borderwidth=0, highlightthickness=0)
        ...
        lf1 = ttk.LabelFrame(content_frame, text="Sections 1-4: File, Material & Constraint", padding="10")
        lf1.grid(row=0, column=0, sticky="ew", columnspan=2)
        lf1.columnconfigure(1, weight=1)

        # --- WIDGET ASSIGNMENT ---
        # Ensure the returned widgets are assigned to instance attributes
        self._create_entry_with_button(
            lf1, "Output File:", 'OUTPUT_FILE', 0, self._browse_save, "Save As...")

        self.spectra_lbl, self.spectra_entry, self.spectra_btn = self._create_entry_with_button(
            lf1, "Spectrum File:", 'SPECTRA', 1, self._browse_spec_file, "Browse...")
        self.spectra_edit_btn = ttk.Button(lf1, text="Create/Edit Spectrum...", command=self._open_spectrum_editor)
        self.spectra_edit_btn.grid(row=1, column=3, padx=5, pady=5, sticky='w')

        self._create_entry_row(
            lf1, "Material Name:", 'MAT', 2, tooltip_text="A descriptive name for the material.")

        self._create_entry_row(
            lf1, "Yield Strength (SYIELD):", 'SYIELD', 3, col=0, tooltip_text="The 0.2% offset yield strength of the material.", numeric=True)
        self._create_entry_row(
            lf1, "Ultimate Strength (SULT):", 'SULT', 3, col=2, tooltip_text="The ultimate tensile strength of the material.", numeric=True)
        self._create_entry_row(
            lf1, "Elastic Modulus (E):", 'E', 4, col=0, tooltip_text="The modulus of elasticity.", numeric=True)
        self._create_entry_row(
            lf1, "Poisson's Ratio (ETA):", 'ETA', 4, col=2, tooltip_text="Poisson's ratio.", numeric=True)

        self._create_entry_row(
            lf1, "Constraint Factor (ALP):", 'ALP', 5, col=0, tooltip_text="Constraint factor, alpha (1.0 for plane stress, ~2.5 for plane strain).", numeric=True)
        self._create_entry_row(
            lf1, "Comp. Yield (BETAT):", 'BETAT', 5, col=2, tooltip_text="Compressive yielding factor for intact material.", numeric=True)
        self._create_entry_row(
            lf1, "Wake Yield (BETAW):", 'BETAW', 6, col=0, tooltip_text="Compressive yielding factor for material in the crack wake.", numeric=True)
        
        self._create_combo_row(
            lf1, "NALP:", 'NALP_DESC', 6, list(self.nalp_map.keys()), col=2,
            tooltip_text="Option for plane-strain to plane-stress transition.\n0: Constant constraint (ALP).\n1: Variable constraint.")
        
        self._create_combo_row(
            lf1, "NEP:", 'NEP_DESC', 7, list(self.nep_map.keys()), col=0,
            tooltip_text="Option for plasticity model.\n1: Plasticity-corrected analysis (default).\n0: Elastic analysis (no plasticity correction).")
    ...
    def _create_tab3(self):
        ...
        # Section 9
        self._create_entry_row(lf9, "NIPT:", "NIPT", 0, 0, tooltip_text="Print interval. Results are printed every NIPT calculated points.\nIf 0, results are printed only at the end of life.", numeric=True)
        self._create_entry_row(lf9, "NPRT:", "NPRT", 0, 1, tooltip_text="Print option. Results are printed for every NPRT-th point saved.\nIf 0, all saved points are printed.", numeric=True)
    ...
    def _update_nfopt_widgets(self):
        ...
        # Update INVERT parameter tooltip based on NFOPT
        is_spectrum = nfopt_code in ['4', '5', '6', '7', '8', '9', '10']
        invert_tip = "For spectrum loads, inverts the sign of all Smin values if set to 1." if is_spectrum else "Not used for this NFOPT."
        self.invert_tooltip.text = invert_tip

        # --- WIDGET NAMES ---
        # Show spectrum file controls on Tab 1 only when needed
        for widget in [self.spectra_lbl, self.spectra_entry, self.spectra_btn, self.spectra_edit_btn]:
            widget.grid() if is_spectrum else widget.grid_remove()
        ...
        # For NFOPT 0 or 1, show the button to open the dedicated block editor
        if nfopt_code in ['0', '1']:
            self.block_editor_button_frame.pack(fill='x')
        else: 
            # For all other spectra, show the standard frame
    ...
    def _initiate_run(self):
        ...
        # Create the log window, which now correctly defines self.log_notebook
    ...
    def _create_log_window(self):
        ...
        # Create the notebook as an INSTANCE ATTRIBUTE
        ...
        fig = Figure(dpi=100)
        fig.set_tight_layout(True)
        # Create the plot axes as an INSTANCE ATTRIBUTE
    ...
    def _process_log_queue(self):
        """Checks queue for messages, updates log, and opens post-processor when done."""
        try:
            while True:
                ...
                if line is None: 
                    ...
                    # Update the plot one last time with the final data points
    ...
    def _parse_output_file(self, filepath=None):
        ...
        try:
            with open(filepath, 'r') as f:
                for line in f:
                    ...
                    if "BLOCK" in stripped_line and "C_crack" in stripped_line:
                        # Use a more robust way to get header, handles potential extra spaces
                        header = [h for h in stripped_line.split(' ') if h]
                    ...
                    if in_data_section:
                        ...
                        try:
                            # Check if the first element is an integer (positive or negative)
                            int(parts[0])
                            
                            # If the last element is an asterisk, remove it
                            if parts[-1] == '*':
                                parts.pop()
            ...
            if not data_table:
                messagebox.showwarning("Parsing Warning", "Could not find a valid data table in the selected file.", parent=self)
                return None, None
