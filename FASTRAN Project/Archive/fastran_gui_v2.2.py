...
class SpectrumCreatorWindow(tk.Toplevel):
    ...
    def _generate_spectrum(self):
        ...
        try:
            title = self.title_entry.get(); invert = int(self.invert_entry.get()); iclip = int(self.iclip_entry.get())
            levels = []; total_cycles = 0; overall_smax = -float('inf'); overall_smin = float('inf')
            
            for row in self.level_widgets:
                smax = float(row[0].get()); smin = float(row[1].get()); cycles = int(row[2].get())
                if cycles < 1: continue
                levels.append({'smax': smax, 'smin': smin, 'cycles': cycles})
                total_cycles += cycles; overall_smax = max(overall_smax, smax); overall_smin = min(overall_smin, smin)

            total_points = total_cycles * 2
            
            with open(self.output_filepath, 'w') as f:
                f.write(f"{title}\n")
                smax_header = int(round(overall_smax)); smin_header = int(round(overall_smin))
                ...
                col_width = spec['width']; cols_per_line = spec['count']

                all_points = []; line_str = ""; current_col = 0
                for level in levels:
                    for _ in range(level['cycles']):
                        all_points.append(level['smax']); all_points.append(level['smin'])
...
class FastranGui(tk.Tk):
    def __init__(self):
        ...
        self.geometry("900x800") # Increased height slightly for status bar
        ...
        self._init_vars()
        self._create_widgets()
        self._load_config() # Load saved settings on startup
    ...
    def _update_all_states(self, *args):
        ...
        for widget in [self.spec_entry, self.browse_spec_button, self.spec_label]: widget.config(state=spec_state)
    ...
    def _initiate_run(self):
        ...
        # 1. Check if FASTRAN path is set
        ...
        # 2. Check and fix the output filename if needed
    ...
    def _create_log_window(self):
        """Creates a Toplevel window to display the live FASTRAN output and controls."""
        ...
        log_text = tk.Text(text_frame, wrap='word', height=20, width=100, font=("Courier", 9))
        ...
        try:
            while True:
                ...
                if line is None: # Sentinel found
                    ...
                    self.save_results_button.config(state="normal") # Enable the save button
                    return
                else:
