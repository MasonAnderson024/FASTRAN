...
[ Growth Rate Equation & Properties ]
  ...
  C1-C7, KF, m
  - These are the coefficients, powers, and toughness parameters for the selected 
    crack growth equation (FASTRAN or NASGRO).
...
class DkeffWindow(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.ntyp_var.trace_add('write', self._update_test_type_widgets)
        ...
        self.output_filename_var = tk.StringVar(value="dkeff_output.dkout")
    ...
    def _create_widgets(self):
        ...
        self.dkeff_version_combo = ttk.Combobox(analysis_lf, textvariable=self.dkeff_version_var, state='readonly', values=["dkeff21f (New)", "dkeff (Legacy)"])
    ...
    def _load_dkeff_input_file(self):
        ...
        try:
            ...
            if len(lines) > 1: self.parent.vars['MAT'].set(lines[1].strip())
    ...
    def _dkeff_worker(self, input_path, output_path):
        ...
        try:
            ...
            # --- Send inputs based on selected version ---
            if "dkeff21f" in dkeff_version:
                # Send the full sequence for the new version
                ...
                process.stdin.write("1\n") # 6. For "DELTA-KEFF(1)...?"
                process.stdin.flush()
            else: # Assume dkeff (Legacy)
                # Send the shorter, original sequence
                process.stdin.write(f"{os.path.basename(input_path)}\n") # 1. Input filename
                process.stdin.flush()
                process.stdin.write(f"{os.path.basename(output_path)}\n") # 2. Output filename
                process.stdin.flush()
...
class FastranGui(tk.Tk):
    ...
    def get_default_data(self):
        return {
            'OUTPUT_FILE': 'output.fout', 'SPECTRA': 'cstamp.txt', 'MAT': 'material name',
    ...
    def _set_input_file(self):
        ...
        if filepath:
            ...
            self.vars['OUTPUT_FILE'].set(f"o_{base_name}.fout")
