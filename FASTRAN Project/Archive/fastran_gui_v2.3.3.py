...
def generate_fastran_file(values, table_data, block_data, save_path, maps):
    try:
        ## Debug line
        # print(f"--- DEBUG: Now inside generate_fastran_file, trying to write to: {save_path} ---")
...
class SpectrumCreatorWindow(tk.Toplevel):
    ...
    def _save_file(self):
        """
        Saves the spectrum file. For normalized values, it scales them
        by a factor of 1000 to maintain precision with the required integer format.
        """
        ...
        try:
            scale_factor = 1000.0 # Factor to convert normalized decimals to integers
            
            title = self.title_entry.get()
            invert = int(self.invert_var.get())
            levels = []
            total_cycles = 0
            overall_smax_int = -float('inf')
            overall_smin_int = float('inf')
            
            self._sync_data_from_widgets()

            for level_data in self.levels_data:
                # Get smax and smin as floats from the entry fields
                smax_float = float(level_data[0])
                smin_float = float(level_data[1])
                cycles = int(level_data[2])
                if cycles < 1: continue

                # Scale to integer and store for writing
            ...
            with open(self.output_filepath, 'w') as f:
                f.write(f"{title}\n")
                # The header uses the scaled integer min/max values
                f.write(f" {total_points}    {overall_smax_int}    {overall_smin_int}    {invert}    3\n")
                ...
                for level in levels:
                    for _ in range(level['cycles']):
                        line_str += f"{level['smax']:{col_width}d}"
                        current_col += 1
                        if current_col >= cols_per_line:
                            f.write(line_str + "\n"); line_str, current_col = "", 0
                        
                        line_str += f"{level['smin']:{col_width}d}"
                        current_col += 1
                        if current_col >= cols_per_line:
                            f.write(line_str + "\n"); line_str, current_col = "", 0
            ...
            messagebox.showinfo("Success", "Spectrum file successfully saved!\nNormalized values were scaled to integers.", parent=self)
    ...
    def _set_output_file(self):
        filepath = filedialog.asksaveasfilename(
            ...
            defaultextension=".spct",
            filetypes=(("Spectrum Files", "*.spct"), ("All Files", "*.*"))
        )
        if filepath:
...
class DkeffWindow(tk.Toplevel):
    ...
    def _validate_grid_data(self):
        """
        Checks if the dK and da/dN columns are in ascending order and styles
        invalid rows with red text. Returns True if data is valid.
        """
        is_valid = True
        last_dk = -float('inf')
        last_rate = -float('inf')

        # Create the 'Invalid.TEntry' style if it doesn't exist
        style = ttk.Style()
        style.configure('Invalid.TEntry', foreground='red')

        for row_widgets in self.grid_widgets:
            dk_entry, rate_entry = row_widgets[0], row_widgets[1]
            row_is_ok = True
            try:
                current_dk = float(dk_entry.get())
                current_rate = float(rate_entry.get())

                if current_dk <= last_dk or current_rate <= last_rate:
                    row_is_ok = False
                    is_valid = False
                ...
                row_is_ok = False # Treat non-numeric rows as invalid
                is_valid = False
            
            # Apply or remove the invalid style
            for widget in row_widgets:
                widget.config(style='Invalid.TEntry' if not row_is_ok else 'TEntry')
        
        if is_valid:
            self.status_label.config(text="Data validation passed.")
        else:
            self.status_label.config(text="Validation Failed: Data must be numeric and in ascending order.")
    ...
    def _load_dkeff_input_file(self):
        """
        Loads a dkeff input file. If multiple datasets are found, it
        prompts the user to select which one to load into the GUI.
        """
        filepath = filedialog.askopenfilename(
            title="Select dkeff Input File",
            filetypes=(("dkeff Input Files", "*.dkin"),("Text Files", "*.txt"), ("All Files", "*.*")),
            parent=self
        )
        if not filepath:
            return
        ...
        try:
            ...
            # --- Scan for dataset headers ---
            datasets = []
            for i, line in enumerate(lines):
                parts = line.split()
                try:
                    # A dataset header has 4 or 5 numeric parts, starting with MTAB
                    is_header = (len(parts) in [4, 5] and all(p.replace('.', '', 1).replace('-', '', 1).isdigit() for p in parts))
                    if is_header:
                        desc = f"Dataset {len(datasets)+1}: "
                        if len(parts) == 5: # Constant R
                            desc += f"R={parts[1]}, Smax={parts[2]}"
                        else: # Kmax
                            desc += f"Kmax={parts[1]}"
            ...
            if not datasets:
                raise ValueError("No valid datasets found in the file.")
            ...
            if len(datasets) > 1:
                ...
                selected_dataset_index = dialog.result_index
            
            # --- Parse the selected dataset ---
            dataset_to_load = datasets[selected_dataset_index]
            start_index = dataset_to_load['start_line']
            ...
            self.parent.vars['MAT'].set(base_name)
            
            parts_l3 = lines[2].split()
            ntyp_code, lunit_code = parts_l3[0], parts_l3[1]
            self.ntyp_var.set(self.ntyp_rev_map.get(ntyp_code, "Compact C(T)"))
            self.lunit_combo.set(next((k for k, v in self.lunit_map.items() if v == lunit_code), "Keep Same Units"))
            ...
            self.nsop_var.set(self.nsop_rev_map.get(parts_l4[6], "Input c (NSOP=1)"))

            parts_header = lines[start_index].split()
            if len(parts_header) == 5: # Constant R
                self.test_type_var.set("Constant R test")
                mtab, r_val, smax_val, w_val, t_val = parts_header
                self.r_var.set(r_val)
                self.smax_var.set(smax_val)
            else: # Kmax
                self.test_type_var.set("Kmax test")
                mtab, kmax_val, w_val, t_val = parts_header
            ...
            loaded_data = []
            data_lines = lines[start_index + 1 : start_index + 1 + mtab]
            ...
            self.output_filename_var.set(f"{os.path.splitext(os.path.basename(filepath))[0]}.dkout")
            self.status_label.config(text=f"Successfully loaded {dataset_to_load['description']}.")
    ...
    def _save_dkeff_input_file(self, filepath):
        try:
            ...
            # dkeff13 has a different, shorter format for Kmax tests
            if "dkeff13" in dkeff_version and test_type == "Kmax test":
                lines.append(f" {syield}  {sult}  {e_mod}  {alp}")
            else:
                # The new dkeff21f and dkeff13's Constant-R test use the longer format
    ...
    def _run_dkeff(self):
        ...
        if not self.parent.dkeff_exe_path:
            messagebox.showerror("dkeff Path Not Set", "Set the path to dkeff.exe in the main window's File menu.", parent=self)
            return

        self.generate_button.config(state="disabled")
        self.status_label.config(text="Status: Running dkeff...")
    ...
    def _dkeff_worker(self, input_path, output_path):
        ...
        try:
            ...
            # --- Use the full command sequence for all versions ---
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                self.dkeff_queue.put("DONE")
                 ...
                 error_message = "dkeff program did not create a valid output file.\n\n"
    ...
    def _process_dkeff_queue(self):
        try:
            ...
            if "ERROR" in message:
                messagebox.showerror("dkeff Error", message, parent=self)
    ...
    def _apply_to_main(self):
        ...
        try:
            # --- Get material properties and perform unit conversion if needed ---
            syield = float(self.syield_entry.get())
            sult = float(self.sult_entry.get())
            e_mod = float(self.e_entry.get())
            
            lunit_code = self.lunit_map[self.lunit_combo.get()]
            conversion_factor = 6.895 # ksi to MPa

            if lunit_code == '1': # Input: English -> Output: SI
                syield *= conversion_factor
                sult *= conversion_factor
                e_mod *= conversion_factor
            elif lunit_code == '2': # Input: SI -> Output: English
                syield /= conversion_factor
                sult /= conversion_factor
                e_mod /= conversion_factor
            ...
            # Update the main window's data structures for the crack growth table
            ...
            messagebox.showinfo("Success", f"Applied {len(self.processed_data)} data points and converted material properties to the main window.", parent=self)
            self.destroy() # Close the generator window
            
        except ValueError:
            messagebox.showerror("Value Error", "Could not apply data. Please ensure material properties are valid numbers.", parent=self)

class BatchInputDialog(tk.Toplevel):
    """
    A dialog window to collect test parameters (Smax, W, T) for multiple R-ratios.
    """
    def __init__(self, parent, r_ratios):
        ...
        # --- Create a scrollable frame for the parameters ---
        ...
        # --- Create headers and entry widgets ---
        headers = ["R-Ratio", "Smax", "Width (W)", "Thickness (T)"]
        for col, header in enumerate(headers):
            ttk.Label(scrollable_frame, text=header, font="-weight bold").grid(row=0, column=col, padx=5, pady=5)

        for i, r_ratio in enumerate(r_ratios, start=1):
            ttk.Label(scrollable_frame, text=f"{r_ratio}").grid(row=i, column=0, sticky='w')
            
            smax_var = tk.StringVar(value="10.0")
            w_var = tk.StringVar(value="3.0")
            t_var = tk.StringVar(value="0.25")
...
class DatasetSelectionDialog(tk.Toplevel):
    """
    A dialog to allow the user to select one of multiple datasets
    found in a single dkeff input file.
    """
    def __init__(self, parent, dataset_info):
        ...
        listbox_frame = ttk.Frame(self, padding=(10, 0, 10, 10))
...
class FastranGui(tk.Tk):
    ...
    def _init_vars(self):
        ...
        self.vars['NTYP_CAT_DESC'] = tk.StringVar(value=list(self.ntyp_categories.keys())[0])
    ...
    def _create_widgets(self):
        ...
        file_menu.add_command(label="Batch Convert .lkpx to dkeff Input...", command=self._batch_convert_lkpx)
        file_menu.add_separator()
    ...
    def _open_dkeff_window(self):
        ...
        if not self.dkeff_exe_path:
            messagebox.showinfo("Path Required", "Please set the path to the dkeff.exe executable first.", parent=self)
    ...
    def _create_tab3(self):
        ...
        self._create_combo_row(lf10, "Specimen Category:", 'NTYP_CAT_DESC', 0, list(self.ntyp_categories.keys()), width=30, tooltip_text="Select a category to filter the specimen types below.")
        ...
        self._create_combo_row(lf10, "Loading Type (LTYP):", 'LTYP_DESC', 2, list(self.ltyp_map.keys()), tooltip_text="Code for loading type.\n0 = tension, 1 = bending, 2 = combined.")
        self._create_combo_row(lf10, "LFAST:", 'LFAST_DESC', 3, list(self.lfast_map.keys()), width=25, tooltip_text="Selects the crack-closure model. Option 0 is the standard model.")
        ...
        self._create_combo_row(lf10, "KCONST:", "KCONST_DESC", 6, list(self.kconst_map.keys()), tooltip_text="Loading control option.\n0 = constant stress loading, 1 = constant SIF range loading.")
        self.ntcmax_lbl, self.ntcmax_combo = self._create_combo_row(lf10, "NTCMAX:", "NTCMAX_DESC", 7, list(self.ntcmax_map.keys()))
    ...
    def _open_and_process_output(self):
        ...
        filepath = filedialog.askopenfilename(
            ...
            filetypes=(("FASTRAN Output Files", "*.fou"), ("Text Files", "*.txt"), ("All Files", "*.*"))
        )
        if not filepath: return
    ...
    def _update_ntyp_options(self, *args):
        """Callback to update the NTYP combobox based on the selected category."""
        try:
            category = self.vars['NTYP_CAT_DESC'].get()
            
            # Get the list of specimen types for the selected category
            filtered_types = self.ntyp_categories.get(category, [])
            
            # Update the values in the NTYP combobox
            self.ntyp_combo['values'] = filtered_types
            
            # Set the selection to the first item in the new list to avoid an invalid state
            if filtered_types:
                # Check if the current value is still valid, if not, reset it
                if self.vars['NTYP_DESC'].get() not in filtered_types:
                    self.vars['NTYP_DESC'].set(filtered_types[0])
            else:
                self.vars['NTYP_DESC'].set("")
        except Exception:
            # This can sometimes be called during initialization before all widgets are fully ready
            pass
    ...
    def _validate_growth_rate_table(self):
        """
        Checks if the dK_eff and da/dN columns in the Section 7 table
        are in ascending order and styles invalid rows with red text.
        """
        is_valid = True
        last_dk = -float('inf')
        last_rate = -float('inf')
        ...
        if is_valid:
            self.status_var.set("Status: Crack growth table validation passed.")
        else:
            self.status_var.set("Status: Validation Failed! Table data must be numeric and in ascending order.")
            ...
            if os.path.exists(self.config_filename):
                with open(self.config_filename, 'r') as f:
                    for line in f:
                        ...
                        # --- Add .strip() to remove whitespace/newlines ---
                        if key == 'fastran_path':
                            self.fastran_exe_path = value.strip()
                        elif key == 'dkeff_path':
                            self.dkeff_exe_path = value.strip()
                
                status_parts = []
                if self.fastran_exe_path:
                    status_parts.append("FASTRAN path loaded.")
                if self.dkeff_exe_path:
                    status_parts.append("dkeff path loaded.")
    ...
    def _save_input_file(self):
        ...
        # print("--- DEBUG: Attempting to save input file... ---")
        if not self.input_filepath:
            self._set_input_file()
        if not self.input_filepath:
            # print("--- DEBUG: Save failed because no filepath was provided. ---")
            return False
    ...
    def _set_input_file(self):
        filepath = filedialog.asksaveasfilename(
            title="Set FASTRAN Input File", 
            defaultextension=".fin", 
            filetypes=(("FASTRAN Input", "*.fin"), ("All Files", "*.*"))
        )
        if filepath:
            self._set_input_path(filepath)
            # Get the filename without the extension
            base_name_no_ext = os.path.splitext(os.path.basename(filepath))[0]
            # Set the default output name based on the input name
            self.vars['OUTPUT_FILE'].set(f"{base_name_no_ext}.fou")
    ...
    def _update_table_from_ntab(self):
        try:
            ...
            self._validate_growth_rate_table()
        except ValueError:
    ...
    def _load_file(self):
        filepath = filedialog.askopenfilename(title="Select FASTRAN Input File", filetypes=(("FASTRAN Input Files", "*.fin"), ("Text Files", "*.txt"), ("All Files", "*.*")))
        if not filepath: return
        ...
        if parsed_data:
            ...
            spectrum_basename = self.vars['SPECTRA'].get()
            if spectrum_basename:
                input_dir = os.path.dirname(filepath)
                potential_path = os.path.join(input_dir, spectrum_basename)
                if os.path.exists(potential_path):
                    self.spectrum_full_path = potential_path
                    self.status_var.set(f"Loaded {os.path.basename(filepath)} and connected spectrum file.")
                else:
                    self.spectrum_full_path = None # Clear path if file not found
    ...
    def _parse_lkpx_for_batch_conversion(self, filepath):
        """
        Parses an .lkpx file to extract material properties and all R-ratio datasets.
        """
        tree = ET.parse(filepath)
        root = tree.getroot()
        ...
        tlookup_table = root.find(".//PropertyData[@property='tlookup']/DataTable")
        if tlookup_table is None:
            raise ValueError("Could not find 'tlookup' data table in the file.")
            
        # Get R-ratio headers and their column positions
        fields = tlookup_table.findall(".//Fields/Field")
        r_map = {field.get('pos'): field.get('property').split('_')[1] for field in fields if field.get('property', '').startswith('r_')}
        
        # Parse data for each R-ratio
        datasets = {r: [] for r in r_map.values()}
        data_rows = tlookup_table.findall(".//Data/row")
        for row_node in data_rows:
            dadn = row_node.find("./FieldData[@pos='1']").text
            for pos, r_val in r_map.items():
                dk = row_node.find(f"./FieldData[@pos='{pos}']").text
                datasets[r_val].append([dk, dadn])
                
        return properties, datasets

    def _batch_convert_lkpx(self):
        """
        Orchestrates the batch conversion of a multi-R-ratio .lkpx file.
        """
        ...
        try:
            mat_props, datasets = self._parse_lkpx_for_batch_conversion(lkpx_path)
        except Exception as e:
            messagebox.showerror("Parsing Error", f"Could not parse the .lkpx file:\n{e}")
            return

        r_ratios = sorted(datasets.keys(), key=float)
        dialog = BatchInputDialog(self, r_ratios)
        test_params = dialog.result

        if not test_params: # User cancelled
            return

        output_path = filedialog.asksaveasfilename(
            title="Save dkeff Input File As",
            defaultextension=".dkin",
        ...
        try:
            with open(output_path, 'w') as f:
                # --- Write Header with clean formatting ---
                f.write(f"Batch conversion from {os.path.basename(lkpx_path)}\n")
                f.write(f" {self.vars['MAT'].get()}\n")
                f.write(" 2  0\n") # NTYP and LUNIT with a leading space and double-space separator
                
                # Material properties line
                syield = float(mat_props['SYIELD'])
                sult = float(mat_props['SULT'])
                e_mod = float(mat_props['E'])
                alp = float(self.vars['ALP'].get())
                ...
                for r_ratio in r_ratios:
                    ...
                    mtab = len(data)
                    # Write dataset header (NSOP=0 format)
                    f.write(f" {mtab}  {float(r_ratio):<4.2f}  {float(smax):<7.1f}  {float(width):<7.4f}  {float(thick):<7.4f}\n")
                    
                    # Write dataset table
                    for i, (dk, dadn) in enumerate(data, start=1):
                        f.write(f"  {i:>2d} {float(dk):>8.4f} {float(dadn):>11.4E}\n")
            
            messagebox.showinfo("Success", f"Successfully created batch input file:\n{os.path.basename(output_path)}")

        except Exception as e:
            messagebox.showerror("File Generation Error", f"Could not write the output file:\n{e}")
