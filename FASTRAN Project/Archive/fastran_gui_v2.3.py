...
class DkeffWindow(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        ttk.Button(top_frame, text="Load .lkpx Material File...", command=self._load_lkpx_file).pack(side='left', padx=(0, 5))
        ttk.Button(top_frame, text="Save Input File...", command=self._save_dkeff_file_as).pack(side='left')
        ...
        ttk.Label(analysis_lf, text="Specimen Width (W):").grid(row=0, column=0, sticky='w', pady=2)
        self.w_entry = ttk.Entry(analysis_lf); self.w_entry.grid(row=0, column=1, sticky='ew', padx=5)
        self.w_entry.insert(0, "3.0")

        ttk.Label(analysis_lf, text="Specimen Thickness (T):").grid(row=1, column=0, sticky='w', pady=2)
        self.t_entry = ttk.Entry(analysis_lf); self.t_entry.grid(row=1, column=1, sticky='ew', padx=5)
        self.t_entry.insert(0, "0.25")

        ttk.Label(analysis_lf, text="Kmax:").grid(row=2, column=0, sticky='w', pady=2)
        self.kmax_entry = ttk.Entry(analysis_lf); self.kmax_entry.grid(row=2, column=1, sticky='ew', padx=5)
        self.kmax_entry.insert(0, "20.0")

        ttk.Label(analysis_lf, text="Constraint Factor (ALP):").grid(row=3, column=0, sticky='w', pady=2)
        self.alp_entry = ttk.Entry(analysis_lf); self.alp_entry.grid(row=3, column=1, sticky='ew', padx=5)
        self.alp_entry.insert(0, "1.8")
        
        ttk.Label(analysis_lf, text="Unit Conversion (LUNIT):").grid(row=4, column=0, sticky='w', pady=2)
        self.lunit_combo = ttk.Combobox(analysis_lf, state='readonly', values=list(self.lunit_map.keys()))
        self.lunit_combo.current(0)
        self.lunit_combo.grid(row=4, column=1, sticky='ew', padx=5)
        ...
        self.grid_frame = ttk.Frame(table_lf)
        ...
        self.generate_button = ttk.Button(bottom_frame, text="Generate dKeff Data", command=self._run_dkeff, state='disabled')
        self.generate_button.pack(side='left', padx=5)
        self.apply_button = ttk.Button(bottom_frame, text="Apply to Main Window", command=self._apply_to_main, state='disabled')
        self.apply_button.pack(side='left', padx=5)
        ttk.Button(bottom_frame, text="Close", command=self.destroy).pack(side='right')


    def _redraw_grid(self, data):
        # This helper function is now used to create the editable grid
        for widget in self.grid_frame.winfo_children():
            widget.destroy()
        self.grid_widgets = []
        
        headers = ['ΔK', 'da/dN', 'Crack Length (c)']
    ...
    def _load_lkpx_file(self):
        """Corrected version that ensures entry fields are writable before inserting text."""
        filepath = filedialog.askopenfilename(title="Select .lkpx Material File", filetypes=(("Material Files", "*.lkpx *.txt"), ("All Files", "*.*")), parent=self)
        if not filepath: return

        try:
            tree = ET.parse(filepath)
            root = tree.getroot()
            ns = {'lk': 'http://www.esrd.com/lkprox'}
            
            # Entry fields are editable before inserting text.
            for entry, path in [(self.syield_entry, './/lk:PropertyData[@property="yld"]/lk:Data'), 
                               (self.sult_entry, './/lk:PropertyData[@property="ult_strength"]/lk:Data'), 
                               (self.e_entry, './/lk:PropertyData[@property="e"]/lk:Data')]:
                entry.config(state='normal')
                entry.delete(0, tk.END)
                entry.insert(0, root.findtext(path, '0.0', namespaces=ns))
            
            # --- Parse and display the data table ---
            loaded_data = []
            for row_node in root.findall('.//lk:DataSet/lk:Row', namespaces=ns):
                dk = row_node.find('./lk:Col[@id="DeltaK"]', ns).text
                dadn = row_node.find('./lk:Col[@id="da_dN"]', ns).text
                # A placeholder for the required crack length 'c'
                loaded_data.append([dk, dadn, ""]) 

            self._redraw_grid(loaded_data)
            self.status_label.config(text=f"Loaded {len(loaded_data)} data points. Please verify and enter crack lengths.")
        except Exception as e:
            messagebox.showerror("File Load Error", f"An error occurred while loading the file:\n{e}", parent=self)

    def _save_dkeff_input_file(self, filepath):
        try:
            lines = [
                f"dkeff13 input from {os.path.basename(filepath)}",
                self.parent.vars['MAT'].get(),
                "2 " + self.lunit_map[self.lunit_combo.get()]
            ]
            syield = self.syield_entry.get(); sult = self.sult_entry.get(); e_mod = self.e_entry.get(); alp = self.alp_entry.get()
            lines.append(f" {syield} {sult} {e_mod} {alp}")

            grid_data = [[widget.get() for widget in row] for row in self.grid_widgets]
            
            kmax = self.kmax_entry.get(); w_val = self.w_entry.get(); t_val = self.t_entry.get()
            lines.append(f" {len(grid_data)} {kmax} {w_val} {t_val}")

            for i, row in enumerate(grid_data):
                dk, dadn, c = row[0], row[1], row[2]
                lines.append(f" {i+1} {dk} {dadn} {c}")
    ...
    def _save_dkeff_file_as(self):
        ...
        if filepath:
            self._save_dkeff_input_file(filepath)

    def _run_dkeff(self):
        if not self.dkeff_input_path:
            messagebox.showerror("File Not Saved", "Please save the dkeff13 input file first.", parent=self)
            return
        if not self.parent.dkeff_exe_path:
            messagebox.showerror("dkeff13 Path Not Set", "Please set the path to dkeff13.exe in the main window's File menu.", parent=self)
            return

        self.generate_button.config(state="disabled")
        self.status_label.config(text="Status: Running dkeff13...")

        self.temp_dir = tempfile.TemporaryDirectory(dir=os.path.dirname(self.dkeff_input_path))
        dkeff_output_path = os.path.join(self.temp_dir.name, "dkeff_output.txt")
        
        thread = threading.Thread(target=self._dkeff_worker, args=(self.dkeff_input_path, dkeff_output_path), daemon=True)
        thread.start()
    ...
    def _dkeff_worker(self, input_path, output_path):
        try:
            ...
            if os.path.exists(output_path): self.dkeff_queue.put("DONE")
            else:
    ...
    def _process_dkeff_queue(self):
        try:
            ...
            if "ERROR" in message:
                ...
                self.status_label.config(text="Status: dkeff13 run complete. Parsing results next.")
            self.generate_button.config(state="normal")
    ...
    def _apply_to_main(self):
        messagebox.showinfo("Info", "Applying data to the main window will be implemented last.", parent=self)
